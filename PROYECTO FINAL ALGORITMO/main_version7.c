...
/*void estadisticas(char id[], char genero[])*/
void menu();
void filmsPerUser();
void addUser();
void addMovie();
...
void historialMovieSupport()
{
	...
	char test[100],*cortador,*cut,limit[]="\t",masUsuario;
			...
			while(fgets(p,100,fHistorial)!=NULL)
			{
				...
				while(cortador != NULL)
				{
					...
					else if (c==1)
					{
						...
						strcpy(conejo.dr,cortador);

					}
					else if (c==2)
					{
						strcpy(conejo.ac,cortador);
					}
					else if (c==3)
					{
						strcpy(conejo.te,cortador);
					}
					else if (c==4)
					{
						strcpy(conejo.av,cortador);
					}
					else if (c==5)
					{
						strcpy(conejo.ot,cortador);
					}
					else if (c==6)
					{
						strcpy(conejo.total,cortador);
					}
					cortador = strtok(NULL,limit);
					c++;
				}
				c=0;
				printf("%7s\t%5s\t%5s\t%5s\t%5s\t%7s\t%8s\n",usuarios.idUsuario,conejo.dr,conejo.ac,conejo.te,conejo.av,conejo.ot,conejo.total);
			}
			fclose(fHistorial);
			break;
		}
