...
int eco=0;//PAra romper el bucle de la validacion ID usuario
struct datosUsuarioscmp{//Para comparar el ID Usuario
char idUsuario[5];
}comparar;

int eco1=0;//Para romper el bucle de validacion Id Pelicula

struct datosPeliculascmp{//Para comparar el ID Pelicula
	char idPelicula[5];
}compararPel;
...
typedef struct estado{//Estrutura para las categorias vistas por el usuario.
	int dr;
	int ac;
	int te;
	int av;
...
typedef struct peliculasvistacantidad{//ESTRUCTURA para todas las peliculas dividas por c
	char dr[10];
	char ac[10];
	char te[10];
	char av[10];
