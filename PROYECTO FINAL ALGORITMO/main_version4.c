...
void pushFilm(char idUsuario[], char idPelicula[]);
...
void historialMovie(){
 ...
 /*Contar el numero de lineas que tiene un archivo guiandonos del ultimo \n de cada oracion.
 while(!feof(fUsuarios))
{
  ...
  if(i == '\n')
  {
    lines++;
		...
		printf("\nEl historial del usuario '%s' se agrego exitosamente, redireccionado al menu principal en 2 segundos...\n", historial.idUsuario);
		sleep(2);
...
void films(char idUsuario[]){
	...
	strcpy(r,idUsuario);
	if ((fUsuarios = fopen(registroPelicula,"r"))==NULL)
	{
		printf("Error accediendo al archivo.\n");
	}
	while(fgets(test,100,fUsuarios)!=NULL)
	{
		...
		{
			pushFilm(r,peliculas.idPelicula);
		}
	}
	fclose(fUsuarios);
}
